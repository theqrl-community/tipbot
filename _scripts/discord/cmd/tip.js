      ...
      tipbotInfo(userID).then(function(tipingUserInfo) {
        ...
        if (botUserCount > 0) {
          if (botUserCount == 1 && message.mentions.users.first() == botId) {
            ...
            errorMessage({ error: 'Tipbot Can\'t be Tipped!', description: 'Tipbot is not accepting tips at this time, If you would like to donate to the tipbot please send a transaction to the faucet address `' + config.discord.prefix + 'send {AMOUNT}' + config.faucet.faucet_wallet_pub + '`' });
          }
          else {
